...
const SHEET_RANGE = 'NiftyMonthly!B2'; // Replace with the range you want to update in the Google Sheet
...
async function fetchDataAndStoreInDB() {
    ...
    try {
        ...
        await connection.query('CALL NF_Insert_ChgInOIVolume_Monthly()');
...
async function fetchDataFromDB() {
    ...
    try {
        connection = await mysql.createConnection(dbConfig);
        const [rows] = await connection.query('SELECT * FROM ChgInOIVolume_Monthly'); // Replace with your query
        return rows;
    } catch (err) {
